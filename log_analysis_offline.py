...
# Function to load logs from directories
def load_logs():
    global log_data
        ...
        if os.path.isdir(dev_path):
            for file in os.listdir(dev_path):
                if file.endswith(".log"):  # Only process log files
                    ...
                    if file_path not in processed_files:  # Check if file was already processed
...
# Function to process logs and perform analysis
def process_logs():
    load_logs()
    ...
    def get_suggestion(log_message, df):
        query_embedding = model.encode([log_message])
        _, closest_idx = index.search(query_embedding, 1)

        if closest_idx[0][0] < len(df):  # Ensure index is within bounds
    ...
    df["suggestion"] = df["log"].apply(lambda log: get_suggestion(log, df))
...
# -------------------------------
# STEP 7: Monitor for New Files
# -------------------------------

def monitor_logs(interval=10):
    while True:
        process_logs()
        time.sleep(interval)
